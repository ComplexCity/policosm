...
def convertGraphCoord(graph):
    ...
    print epsg
    return int(epsg), convertNodesCoordinates(graph, 4326, int(epsg))
...
if __name__ == "__main__":
    ...
    res = kernel.score_samples(zip(*positions[2]))

    with open('roads_centroids_Vernon500.csv', 'wb') as csvfile:
        writer = csv.writer(csvfile)
        for idx, coord in enumerate(zip(*positions[2])):
            converted_coord = convertCoordinates(coord, epsg, 4326)
            writer.writerow([converted_coord[0], converted_coord[1], res[idx]])
